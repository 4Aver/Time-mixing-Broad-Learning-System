...
from 研究生课题.集成学习与宽度学习.MBLS.model.wo4_RTBLS import wo4_RTBLS
...
def read_npy(type_name,epochs,choice_MLP,input_dim,time_len,map_fea_num, map_num, enh_fea_num, enh_num,weight_decay,n_block=3):
    # 切分数据
    ...
    # 定义模型
    model = wo4_RTBLS(choice_MLP=choice_MLP,input_dim=input_dim,time_len=time_len,n_block=n_block,
                 map_fea_num=map_fea_num, map_num=map_num, enh_fea_num=enh_fea_num, enh_num=enh_num)
    ...
    train_out_map_features,val_out_map_features,test_out_map_features = 0,0,0
    # 训练模型
    ...
    get_all_result(best_test_target, best_test_output)
    ...
    data_out.to_csv(r'C:\Users\86178\Desktop\课题\论文\MBLS\最终实验\{}\{}_wo_RTBLS.csv'.format(type_name,type_name))
...
# read_npy('lorenz',epochs = 300,input_dim=15,choice_MLP=True,time_len=5,map_fea_num=6, map_num=10, enh_fea_num=40, enh_num=5,n_block=1,weight_decay=0.05)
# read_npy('rossler',epochs = 300,choice_MLP=True,input_dim=13,time_len=5,map_fea_num=15, map_num=3, enh_fea_num=1, enh_num=2,n_block=1,weight_decay=0.05)

# read_npy('sea_clutter', epochs = 500,choice_MLP=True,input_dim=59,time_len=5,map_fea_num=51, map_num=3, enh_fea_num=1, enh_num=2,n_block=2,weight_decay=0.1)
# read_npy('sun_spot', epochs = 500,choice_MLP=True,input_dim=5,time_len=5,map_fea_num=15, map_num=3, enh_fea_num=1, enh_num=2,n_block=2,weight_decay=0.1)
...
def choice(type_name,input_dim,map_fea_num, map_num, enh_fea_num, enh_num,hidden_dim1=64):
    ...
    for i in [0,1,2,3,4]:
        ...
        read_npy(type_name,input_dim=input_dim,choice_MLP=True,time_len=5,
                     map_fea_num=map_fea_num, map_num=map_num, enh_fea_num=enh_fea_num, enh_num=enh_num,
                 n_block=1,weight_decay=0.002,epochs=300)
        end_time = time.time()
