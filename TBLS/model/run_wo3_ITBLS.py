...
from 研究生课题.集成学习与宽度学习.MBLS.model.wo3_ITBLS import wo3_ITBLS
...
def read_npy(type_name,epochs,list_input_dim,choice_MLP,time_len,map_fea_num, map_num, enh_fea_num, enh_num,weight_decay,lr = 0.001,hidden_dim=128):
    # 切分数据
    ...
    # 定义模型
    model = wo3_ITBLS(list_input_dim=list_input_dim,choice_MLP=choice_MLP,time_len=time_len,map_fea_num=map_fea_num, map_num=map_num, enh_fea_num=enh_fea_num, enh_num=enh_num,hidden_dim=hidden_dim)
    ...
    data_out.to_csv(r'C:\Users\86178\Desktop\课题\论文\MBLS\最终实验\{}\{}_wo_ITBLS.csv'.format(type_name,type_name))
    end_time = time.time()
    print('计算所花费的时间为：', end_time - start_time)

    plt.plot(best_test_target, label='true')
    plt.plot(best_test_output, label='pred')
    plt.legend()
    plt.show()
...
# read_npy('lorenz',epochs = 300,list_input_dim=[5,5,5],choice_MLP=False,time_len=5,map_fea_num=10, map_num=6, enh_fea_num=40, enh_num=5,weight_decay=0.02,lr = 0.0005,hidden_dim=64)
read_npy('rossler',epochs = 300,list_input_dim=[3,5,5],choice_MLP=True,time_len=5,map_fea_num=10, map_num=6, enh_fea_num=30, enh_num=5,weight_decay=0.02,lr = 0.0005,hidden_dim=64)


# list_input_dim = [4,2,5,5,5,3,5,5,5,5,5,3,3,4]
# read_npy('sea_clutter',epochs = 500,list_input_dim=list_input_dim,choice_MLP=True,time_len=5,map_fea_num=10, map_num=2, enh_fea_num=40, enh_num=2,weight_decay=0.02,lr = 0.001)

# read_npy('sun_spot',epochs = 500,list_input_dim=[5],choice_MLP=False,time_len=5,map_fea_num=15, map_num=3, enh_fea_num=1, enh_num=1,weight_decay=0.02,lr = 0.001)
