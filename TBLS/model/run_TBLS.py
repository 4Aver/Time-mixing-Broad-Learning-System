...
from 研究生课题.集成学习与宽度学习.MBLS.model.TBLS import MBLS
...
def get_data(x, y, time_len,data_len,split_size,random_state=42):
...
def read_npy(type_name,epochs,choice_MLP,input_dim,time_len,map_fea_num, map_num, enh_fea_num, enh_num,weight_decay,random_state=42,n_block=3,lr = 0.001,hidden_dim1=256,train_num=1):
    # 切分数据
    if type_name=='sun_spot':
        ...
        data_len,split_size = 2700, 0.975
    else:
    ...
    # 定义模型
    model = MBLS(choice_MLP=choice_MLP,input_dim=input_dim,time_len=time_len,n_block=n_block,
    ...
    # torch.save(best_model.state_dict(),r'./save_best_model/best_model_{}.pth'.format(type_name))

    data_loss = pd.DataFrame({'trian_loss':list_train_loss,'test_loss':list_test_loss})
    # data_loss.to_csv(r'C:\Users\86178\Desktop\课题\论文\TBLS\最终实验\lorenz\误差\{}_TBLS_loss.csv'.format(type_name))
    ...
    # data_out.to_csv(r'C:\Users\86178\Desktop\课题\论文\TBLS\最终实验\{}\{}_TBLS.csv'.format(type_name,type_name))
    # data_out.to_csv(r'C:\Users\86178\Desktop\课题\论文\TBLS\最终实验\多次对比实验\{}\TBLS\{}_TBLS_{}.csv'.format(type_name,type_name,train_num))
...
def choice_param():
    ...
    list_loss = []
    s=0
    for map_fea_num in [10]:
        for map_num in [7]:
            for enh_fea_num in [10]:
                for enh_num in [5,6,7]:
                    print('第{}次寻找最优参数'.format(s))
                    loss = {}
                    mse, r2 = read_npy(type_name,epochs = 50,input_dim=15,choice_MLP=True,time_len=5,map_fea_num=map_fea_num, map_num=map_num, enh_fea_num=enh_fea_num, enh_num=enh_num,n_block=1,weight_decay=0.02,lr = 0.001)

                    loss['map_fea_num'] = map_fea_num
                    loss['map_num'] = map_num
                    loss['enh_fea_num'] = enh_fea_num
                    loss['enh_num'] = enh_num
                    loss['mse'] = mse
                    loss['r2'] = r2
    ...
    df_sorted = data_loss.sort_values('mse', ascending=True)
    # df_sorted.to_csv(r'C:\Users\86178\Desktop\课题\论文\TBLS\最终实验\{}_TBLS.csv'.format(type_name))
    print(df_sorted)


if __name__ == '__main__':
    def choice(type_name,input_dim,map_fea_num, map_num, enh_fea_num, enh_num):
        list_time = []
        for i in range(5):
            start_time = time.time()
            read_npy(type_name,random_state = i,train_num = i+1,epochs = 30,input_dim=input_dim,choice_MLP=True,time_len=15,
                     map_fea_num=map_fea_num, map_num=map_num, enh_fea_num=enh_fea_num, enh_num=enh_num,n_block=1,weight_decay=0.002,lr = 0.001,hidden_dim1=64)
            end_time = time.time()
            cost_time = end_time - start_time
            print('计算所花费的时间为：', end_time - start_time)
            list_time.append(cost_time*10)
        data_time = pd.DataFrame(list_time)
        data_time.to_csv(r'C:\Users\86178\Desktop\课题\论文\TBLS\最终实验\多次对比实验\{}\TBLS\{}_TBLS_time.csv'.format(type_name,type_name))
